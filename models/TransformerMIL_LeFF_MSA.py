...
#test 9:CMCPE(cnn_feat+proj+proj1+proj2)+Performer+MIL,the CMCPE(cnn_feat+proj+proj1+proj2)
class CMCPE(nn.Module):
   def __init__(self, dim=512):
       super(CMCPE, self).__init__()
   ...
   def forward(self, x, H, W):
       ...
       x = self.proj(cnn_feat)+cnn_feat+self.proj1(cnn_feat)+self.proj2(cnn_feat)                          #cnn_feat add to the (obtained from convolution block processing with kernal k=1,3,5 padding=0,1,2)
...
class TransformerMIL(nn.Module):
    def __init__(self, n_classes):
        ...
        self.pos_layer1 = CMCPE(dim=512)
        self.layer2 = TransLayer(dim=512)
        self.pos_layer2 = CMCPE(dim=512)
        #add another layer
