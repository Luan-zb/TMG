...
#        class_token_attention_scores=h[:,1:H, :1]
#        print("class_token_attention_scores shape:",class_token_attention_scores.shape)
#        class_token_to_remain_tokens=class_token_attention_scores.transpose(1,0)[0][0:1]
...
#            return class_token_to_remain_tokens
#        B, _, C = h.shape
#        h=h[:,1:].expand(1, -1, -1).transpose(1, 2).view(B, C, _H, _W)   # torch.Size([1, 512, 78, 78])
#        h=torch.nn.functional.adaptive_avg_pool2d(h, (1,1))              # torch.Size([1, 512, 1, 1])
#        h = h.flatten(2).transpose(1, 2)                                 #  torch.Size([1, 1, 512])
...
#        # add the code to evaluate attention score
