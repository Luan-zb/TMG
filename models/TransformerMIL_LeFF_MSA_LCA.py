...
#test 9:LeFF(cnn_feat+proj+proj1+proj2+pro3)+NystromAttention+MIL,the LeFF(cnn_feat+proj+proj1+proj2)-->PPEG,##Attention (4):Performer with SelfAttention##
class LeFF(nn.Module):
   def __init__(self, dim=512):
    ...
    #    self.proj3 = nn.Conv2d(dim, dim, 9, 1, 9//2, groups=dim)
...
class TransformerMIL(nn.Module):
    ...
    def forward(self,h):
        ...
        class_token1=h[:,0].expand(1, -1, -1)
        class_token=torch.cat((cls_tokens,class_token1),dim=1)

        
        # #---->Translayer x2------->MSA(h)
        # h = self.layer2(class_token) #[B, N, 512]
        # # print("after layer2 shape",h.shape)  
        ...
        class_token2=h[:,0].expand(1, -1, -1)

        # print("class_token2 shape:",class_token2.shape)             #class_token2 shape: torch.Size([1, 512])
        class_token=torch.cat((class_token2,class_token1),dim=1)
        # class_token=torch.cat((class_token,cls_tokens),dim=1)


        #---->Translayer x3------->MSA(h)
        h = self.layer3(class_token) #[B, N, 512]


        # CNN VS NIN:https://www.csdn.net/tags/NtjaYgysNjY4NS1ibG9n.html
        # original CNN procedure
        #---->cls_token
        h = self.norm(h)[:,0]
...
if __name__ == "__main__":
    ...
    model = TransformerMIL(n_classes=5)
    print(model.eval())
