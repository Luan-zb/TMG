...
class LeFF(nn.Module):
   ...
   def forward(self, x, H, W):
    ...
    #    print("x.shape:",x.shape)                    # x.shape torch.Size([1, 6085, 512])
       ...
       feat_token = x  # split x into cls_token-->torch.Size([1, 512]) , feat_token-->torch.Size([1, 6084, 512]) 
...
class TransformerMIL(nn.Module):
    ...
    def forward(self,h):
        ...
        h=h.expand(1, -1, -1).transpose(1, 2)
        h=self.conv1(h)
        ...
        B, C , _= h.shape
        h=h.view(B, C, _H, _W)   # torch.Size([1, 512, 78, 78])
