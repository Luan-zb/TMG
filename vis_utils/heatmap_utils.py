...
def compute_from_patches(wsi_object, clam_pred=None, model=None, feature_extractor=None, batch_size=512,  
    ...
    for idx, (roi, coords) in enumerate(roi_loader):
        ...
        with torch.no_grad():
            ...
            #print("features is on the cuda:",features.device)  #features is on the cuda: cuda:0

            #print("attn_save_path:",attn_save_path)   #attn_save_path: heatmaps/heatmap_raw_results/HEATMAP_OUTPUT/Kidney/C3N-03021-24/C3N-03021-24_0.5_roi_False.h5
            #attn_save_path: heatmaps/heatmap_raw_results/HEATMAP_OUTPUT/Kidney/TCGA-UW-A7GI-01Z-00-DX1.4A1709FC-F8F3-4509-A011-10CBF1B53DA9/TCGA-UW-A7GI-01Z-00-DX1.4A1709FC-F8F3-4509-A011-10CBF1B53DA9_0.5_roi_False.h5

            if attn_save_path is not None:
                A = model(features, attention_only=True)   #=========如果是CLAM和TOAD方法的话需要attention_only的内容进行返回
                #A = model(features)['A']
                #print("A is on the cuda:",model(features, attention_only=True).device)
                print("%%%%%%%%%%%%%%%%")
                print("A shape:",A.shape)
           
                #if A.size(0) > 1: #CLAM multi-branch attention     #若为TOAD和TransMIL方法，则该条件语句则需要进行注释
                #     A = A[clam_pred]
                #     print("A shape:",A.shape)

                A = A.contiguous().view(-1, 1).cpu().numpy()
