...
def savePatchIter_bag_hdf5(patch):
    ...
    print("img_patch",np.array(img_patch).shape)  # (256, 256, 3)
    img_patch = np.array(img_patch)[np.newaxis,...]
    img_shape = img_patch.shape
    print("img_shape",img_shape)   # (1,256,256,3)
...
def savePatchIter_bag_hdf5_after_normalization(patch):
    x, y, cont_idx, patch_level, downsample, downsampled_level_dim, level_dim, img_patch, name, save_path= tuple(patch.values())
    #print("img_patch",img_patch.shape)
    print("img_patch(savePatchIter_bag_hdf5)",np.array(img_patch).shape)  # (256, 256, 3)

    refImage = PIL.Image.open('ref.png')
    refImage = np.array(refImage)
    print("refImage.shape(savePatchIter_bag_hdf5)",refImage.shape)     # (256, 256, 3)
    ...
    try:
        ...
        img_patch = normalizer.transform(np.array(img_patch).astype('uint8'))
        print("color normalization(savePatchIter_bag_hdf5) "+str(x,y))
    ...
    img_shape = img_patch.shape
    print("img_shape(savePatchIter_bag_hdf5)",img_shape)   # (1,256,256,3)
...
def initialize_hdf5_bag_after_normalization(first_patch, save_coord=False):
    x, y, cont_idx, patch_level, downsample, downsampled_level_dim, level_dim, img_patch, name, save_path = tuple(first_patch.values())
    print("img_patch(initialize_hdf5_bag)",np.array(img_patch).shape)
    refImage = PIL.Image.open('ref.png')
    refImage = np.array(refImage)
    print("refImage.shape(initialize_hdf5_bag)",refImage.shape)     # (256, 256, 3)
    refImage.astype('uint8')
    print("refImage.shape 2(initialize_hdf5_bag)",refImage.shape)     # (256, 256, 3)
    normalizer = staintools.StainNormalizer('vahadane')
    normalizer.fit(refImage)
    try:
        img_patch = normalizer.transform(np.array(img_patch).astype('uint8'))   # (256, 256, 3)
        print("img_patch shape(initialize_hdf5_bag)",img_patch.shape)
        print("color normalization(initialize_hdf5_bag) "+str(x)+" "+str(y))
    except TissueMaskException:
        print("Empty tissue mask computed")
    except LinAlgError:
        print('This is an empty tissue mask, skipped.')
    ...
    dset.attrs['level_dim'] = level_dim
    dset.attrs['downsampled_level_dim'] = downsampled_level_dim
    # print("------dset.dims----",dset.shape) #------dset.dims---- (1, 256, 256, 3)
    # print("------dset value----",np.array(dset))

    if save_coord:
        coord_dset = file.create_dataset('coords', shape=(1, 2), maxshape=(None, 2), chunks=(1, 2), dtype=np.int32)
        coord_dset[:] = (x,y)
        # print("------coord_dset.dims----",coord_dset.shape)    #------coord_dset.dims---- (1, 2)
        # print("------coord_dset value----",np.array(coord_dset)) 
