...
import h5py
import math
from wsi_core.wsi_utils import savePatchIter_bag_hdf5, savePatchIter_bag_hdf5_after_normalization,initialize_hdf5_bag,initialize_hdf5_bag_after_normalization, coord_generator, save_hdf5, sample_indices, screen_coords, isBlackPatch, isWhitePatch, to_percentiles
import itertools
...
# add code
# from wsi_core.filtering import min_max_filtering
...
class WholeSlideImage(object):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        #add_code
        # O_P = min_max_filtering(M = 1, N = 20, I = img_hsv[:,:,1])
        # O_P = O_P.astype('uint8')
        #img_med = cv2.medianBlur(O_P, mthresh) 
        ...
        #print("contours, hierarchy",np.array(contours).shape, np.array(hierarchy).shape)
        hierarchy = np.squeeze(hierarchy, axis=(0,))[:, 2:]
        if filter_params: foreground_contours, hole_contours = _filter_contours(contours, hierarchy, filter_params)  # Necessary for filtering out artifacts

        self.contours_tissue = self.scaleContourDim(foreground_contours, scale)
        #print("self.contours_tissue",np.array(self.contours_tissue).shape)  #self.contours_tissue (1, 13856, 1, 2)
        self.holes_tissue = self.scaleHolesDim(hole_contours, scale)        #self.holes_tissue (1, 0)
        #print("self.holes_tissue",np.array(self.holes_tissue).shape)

        contour_ids = set(np.arange(len(self.contours_tissue)))

        self.contours_tissue = [self.contours_tissue[i] for i in contour_ids]
        #print("after self.contours_tissue",np.array(self.contours_tissue).shape)
        self.holes_tissue = [self.holes_tissue[i] for i in contour_ids]
        #print("afetr self.holes_tissue",np.array(self.holes_tissue).shape)
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        #annot_color(red);color(lime);hole_color(BLUE)

        downsample = self.level_downsamples[vis_level]
        print("downsample:",downsample)                  #downsample: (16.001041201844416, 16.000226295541978)
        scale = [1/downsample[0], 1/downsample[1]]
        print("top_left,bot_right:",top_left,bot_right)  #top_left,bot_right: None None
        ...
        print("original feature map:",self.level_dim[0])  #original feature map: (107575, 70705)
        print("region_size:",region_size)                 #region_size: (6723, 4419)
        print("top_left:",top_left)                       #top_left: (0, 0)
        print("vis_level:",vis_level)                     #vis_level: 2
        ...
        #img.save(os.path.join("/data/luanhaijing/project/tissue_process_pipeline/heatmaps/heatmap_production_results/HEATMAP_OUTPUT/Kidney", "original.jpg"), quality=100)
    ...
    def createPatches_bag_hdf5(self, save_path, patch_level=0, patch_size=256, step_size=256, save_coord=True, **kwargs):
        ...
        # print("contours(createPatches_bag_hdf5)",np.array(contours).shape)  #contours(createPatches_bag_hdf5) (1, 4181, 1, 2)
        contour_holes = self.holes_tissue
        # print("contour_holes",contour_holes)
        ...
        for idx, cont in enumerate(contours):
            ...
            print("-------------------------------------")
            print("self.hdf5_file(createPatches_bag_hdf5)",idx,self.hdf5_file)
    ...
    def createPatches_bag_hdf5_after_normalization(self,normalizer,save_path, patch_level=0, patch_size=256, step_size=256, save_coord=True, **kwargs):
        contours = self.contours_tissue
        # contour_ids = set(np.arange(len(contours)))
        # print("contour_ids",contour_ids)
        # contours= [contours[i] for i in contour_ids]
        # print("contours",contours)
        # print("self.contours_tissue",np.array(self.contours_tissue).shape)
        # self.holes_tissue = [self.holes_tissue[i] for i in contour_ids]
        # print("self.holes_tissue",np.array(self.holes_tissue).shape)
        #print("contours(createPatches_bag_hdf5)",np.array(contours).shape)  #contours(createPatches_bag_hdf5) (1, 4181, 1, 2)
        contour_holes = self.holes_tissue
        ...
        elapsed = time.time()
        for idx, cont in enumerate(contours):
            #print("idx cont",np.array(cont).shape)
            patch_gen = self._getPatchGenerator_after_normalization(normalizer,cont, idx, patch_level, save_path, patch_size, step_size, **kwargs)
            #print("--------------patch_gen(createPatches_bag_hdf5)",len(list(patch_gen)))   #<generator object WholeSlideImage._getPatchGenerator at 0x7fd181368dd0>
            #print("self.hdf5_file(createPatches_bag_hdf5)",idx,self.hdf5_file)

            if self.hdf5_file is None:
                try:
                    first_patch = next(patch_gen)
                    print("first_patch(createPatches_bag_hdf5)",first_patch)  #{'x': 13665, 'y': 3361, 'cont_idx': 0, 'patch_level': 0, 'downsample': (1.0, 1.0), 'downsampled_level_dim': (23903, 24732), 'level_dim': (23903, 24732), 'patch_PIL': <PIL.Image.Image image mode=RGB size=256x256 at 0x7FD1812E3910>, 'name': 'C3N-01534-23', 'save_path': 'RESULTS_DIRECTORY/SAR_all_in_one/tissue_patches'}
                ...
                file_path = initialize_hdf5_bag(first_patch, save_coord=save_coord)
                self.hdf5_file = file_path
                #print("self.hdf5_file(createPatches_bag_hdf5 num1)",self.hdf5_file)
            #https://blog.csdn.net/YZL40514131/article/details/119989244
            for patch in patch_gen:
                savePatchIter_bag_hdf5(patch)
            # savePatchIter_bag_hdf5_after_normalization(next(patch_gen))
        return self.hdf5_file

    def _getPatchGenerator_after_normalization(self,normalizer,cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                #normalizer = staintools.StainNormalizer('macenko')  #vahadane
                #print('macenko')
                #normalizer.fit(refImage)
                try:
                    patch_PIL = normalizer.transform(np.array(patch_PIL))
                except TissueMaskException:
                    print("Empty tissue mask computed")
                    continue
                except LinAlgError:
                    print('This is an empty tissue mask, skipped.')
                    continue
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        if use_padding:
            ...
            stop_y = min(start_y+h, img_h-ref_patch_size[1])
            stop_x = min(start_x+w, img_w-ref_patch_size[0])
        print("start_y, stop_y,step_size_y,start_x, stop_x,step_size_x:",start_y, stop_y,step_size_y,start_x, stop_x,step_size_x)
        count = 0
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                'downsample': self.level_downsamples[patch_level], 'downsampled_level_dim': tuple(np.array(self.level_dim[patch_level])//custom_downsample), 'level_dim': self.level_dim[patch_level],
                'patch_PIL':patch_PIL, 'name':self.name, 'save_path':save_path}
                print("patch_info(_getPatchGenerator)",count,x,y,patch_info)
                yield patch_info
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        print("len(scores.shape:",len(scores.shape))
