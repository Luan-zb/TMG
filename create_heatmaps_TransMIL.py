...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):
	...
	with torch.no_grad():
		if isinstance(model, (CLAM_SB, CLAM_MB,TransMIL,TOAD_fc_mtl_concat)):
			...
			results_dict= model(features)#.cuda()#.to("cuda:0")
			#results_dict.relocate()

			#print("result_dict is on cuda:",results_dict.is_cuda)
...
if __name__ == '__main__':
	...
	print("process_stack",process_stack)
	...
	print("reverse_label_dict",reverse_label_dict)
	...
	for i in range(len(process_stack)):
		...
		#Segment the tissue slides and save the segmentation results to pkl
		...
		print("vis_patch_size",vis_patch_size)

		block_map_save_path = os.path.join(r_slide_save_dir, '{}_blockmap.h5'.format(slide_id))
		print("block_map_save_path",block_map_save_path)

		
		mask_path = os.path.join(r_slide_save_dir, '{}_mask.jpg'.format(slide_id))

		wsi_ref_downsample = wsi_object.level_downsamples[patch_args.patch_level]
		print("patch_level",patch_args.patch_level)
		print("wsi_ref_downsample",wsi_ref_downsample)
		
		print("patch_args.custom_downsample",patch_args.custom_downsample)
		...
		#		print("sample_save_dir",sample_save_dir)
